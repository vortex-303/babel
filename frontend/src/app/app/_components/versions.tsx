...
function VersionRow({ job, onChange }: { job: JobRow; onChange: () => void }) {
  ...
  const remove = async () => {
    if (!confirm(`Delete this translation (${job.source_lang} → ${job.target_lang})? This can't be undone.`)) return;
    setBusy(true);
    try {
  ...
  return (
    <li className="px-4 py-2 flex items-center gap-3 flex-wrap">
      ...
      <button
        ...
        className="text-xs px-2 py-0.5 rounded border border-red-300 text-red-700 hover:bg-red-50 dark:border-red-800/50 dark:text-red-300 dark:hover:bg-red-950/30"
      >
        Delete
      </button>
      {job.error && (
